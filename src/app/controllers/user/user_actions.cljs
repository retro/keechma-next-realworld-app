...
(defn optimistic-toggle-follow [value]
  (update value :following not))
...
(def pipelines
  {:logout (pipeline! [value ctrl]
   ...
   :toggle-favorite (-> (pipeline! [value {:keys [deps-state*] :as ctrl}]
                      ...
                      (pp/set-queue #(:slug %))
                      pp/enqueued)
   :toggle-follow (-> (pipeline! [value {:keys [deps-state*] :as ctrl}]
                        (edb/insert-entity! ctrl :entitydb :user (optimistic-toggle-follow value))
                        (let [api-fn (if (:following value) api/follow-delete api/follow-create)]
                          (api-fn {:username (:username value) :jwt (:jwt @deps-state*)}))
                        (edb/insert-entity! ctrl :entitydb :user value))
                    (pp/set-queue #(:username %))
                    pp/enqueued)
   :delete-article (pipeline! [value _]
                     (when (js/confirm "Are your sure?")
                       (let [slug (:slug value)]
                         (pipeline! [value {:keys [deps-state*] :as ctrl}]
                           (api/article-delete {:article-slug slug :jwt (:jwt @deps-state*)})
