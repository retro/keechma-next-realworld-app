...
(def pipelines
  {:keechma.on/start (pipeline! [value {:keys [deps-state*]}]
                       (let [{:keys [entitydb router]} @deps-state*
                         ...
                         (if user
                           (pipeline! [value ctrl]
                             (edb/insert-named! ctrl :entitydb :user :user/profile user)
                             (pp/detached load-user))
                           load-user)))
   :keechma.on/stop (pipeline! [_ ctrl]
                      (edb/remove-named! ctrl :entitydb :user/profile))})

(defmethod ctrl/prep :profile-user [ctrl]
  (pipelines/register ctrl pipelines))

(defmethod ctrl/derive-state :profile-user [_ state {:keys [entitydb]}]
