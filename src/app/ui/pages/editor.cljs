(ns app.ui.pages.editor
  (:require [keechma.next.helix.core :refer [with-keechma dispatch call use-sub use-meta-sub]]
...
(defnc EditorRenderer [props]
  (throw-promise! (use-meta-sub props :editor-form) :keechma.on/start)
  (let [form-state (:keechma.next.controllers.form/form (use-meta-sub props :editor-form))]
    (d/div
      {:class "auth-page"}
      (d/div
        ...
        (d/div
          ...
          (d/div
            ...
            (d/form
              ...
              (d/button
                {:class "btn btn-lg btn-primary pull-xs-right"} "Publish Article"))))))))
