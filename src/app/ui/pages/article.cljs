...
(defnc ArticleMeta [{:keys [article] :as props}]
  (let [author (:author article)
        ...
        author-profile-url (router/get-url props :router {:page "profile" :user (:username author)})]
    (d/div
      {:class "article-meta"}
      (d/a
        {:href author-profile-url}
        (d/img {:src (:image author)}))
      (d/div
        {:class "info"}
        (d/a {:class "author" :href author-profile-url} (:username author))
...
(defnc Banner [{:keys [article] :as props}]
  (d/div
    {:class "banner"}
    (d/div
      {:class "container"}
...
(defnc Tags [{:keys [article] :as props}]
  (d/ul
    {:class "tag-list"}
    (map
      (fn [{:keys [tag]}]
        (d/li
          {:key tag :class "tag-default tag-pill tag-outline"}
          (d/a {:href (router/get-url props :router {:page "home" :tag tag})} tag)))
      (:tagList article))))
    ...
    (d/div
      ...
      (d/div
        ...
        (d/div
          ...
          (d/div
            {:class "col-md-12"}
            (d/div {:dangerouslySetInnerHTML #js{:__html (render-markdown (:body article))}})
            ($ Tags {:article article & props})))
        (d/hr)
        (d/div {:class "article-actions"} ($ ArticleMeta {:article article & props}))
        (d/div
          {:class "row"}
          (d/div
