...
(def email-regex-strict #"[a-zA-Z0-9!#$%&'*+/=?^_`{|}~-]+(?:\.[a-zA-Z0-9!#$%&'*+/=?^_`{|}~-]+)*@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?")
...
(defn email? [v _ _]
  (if (or (nil? v) (empty? v))
    true
    (re-matches email-regex-strict (str v))))
...
(defn numeric-with-decimal? [v _ _]
  (if (nil? v)
    true
...
(def default-validations
  {:not-empty          {:message   "Please enter a value."
   ...
   :bool               {:message   "Please select a value."
                        :validator bool?}
